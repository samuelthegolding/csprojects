...
public class RecursionTest {
    ...
    public void eRecursionTestArrayMinInputTest() {
        ...
        Assert.assertEquals(String.format("ArrayMin must be able to handle position of %d", position), 89, Recursion.arrayMin(inOrder, position));
    }
    ...
    public void gRecursionTestComputePyramidWeightsInputTest() {
        ...
        Assert.assertEquals("ComputePyramidWeights must be able to handle an empty array", 0.0, Recursion.computePyramidWeights(masses1, 0, 0), 0.001);
        Assert.assertEquals("ComputePyramidWeights must be able to handle negative row values", 0.0, Recursion.computePyramidWeights(masses1, -1, 0), 0.001);
        Assert.assertEquals("ComputePyramidWeights must be able to handle negative col values", 0.0, Recursion.computePyramidWeights(masses1, 0, -1), 0.001);
        Assert.assertEquals("ComputePyramidWeights must be able to handle invalid column", 0.0, Recursion.computePyramidWeights(masses2, 0, 3), 0.001);
        Assert.assertEquals("ComputePyramidWeights must be able to handle invalid row", 0.0, Recursion.computePyramidWeights(masses2, 4, 0), 0.001);
    }
